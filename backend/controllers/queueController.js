...
// =====================================================================
// --- PERUBAHAN DARI BREVO KE GMAIL (NODEMAILER) ---
// =====================================================================

// 1. Impor Nodemailer (library baru untuk mengirim email)
const nodemailer = require("nodemailer");

// 2. Hapus atau komentari kode Brevo
...
// 3. Buat transporter Nodemailer untuk Gmail
// Objek ini akan digunakan kembali untuk semua pengiriman email.
const transporter = nodemailer.createTransport({
  host: "smtp.gmail.com",
  port: 587,
  secure: false, // `true` untuk port 465, `false` untuk port lain (menggunakan STARTTLS)
  auth: {
    user: process.env.EMAIL_USER, // Email Anda dari file .env
    pass: process.env.EMAIL_PASS, // Sandi Aplikasi Anda dari file .env
  },
});
...
// Helper function untuk mengirim notifikasi Email (SUDAH DIUBAH MENGGUNAKAN NODEMAILER)
const sendEmailNotification = async (toEmail, subject, htmlContent) => {
  // Definisikan opsi email
  const mailOptions = {
    from: `"Antrian PN Manokwari" <${process.env.EMAIL_USER}>`, // Nama pengirim & email
    to: toEmail,
    subject: subject,
    html: htmlContent,
  };
  ...
  try {
    // Kirim email menggunakan transporter yang sudah dibuat
...
const preparePublicQueueData = async () => {
  ...
  for (const service of services) {
    const queuesForService = activeQueues.filter(
      (q) => q.service_id === service.id
    );
    ...
    if (callingQueue) {
      ...
      const callingQueueDailyNum = parseInt(
        callingQueue.full_queue_number.replace(service.service_prefix, "")
      );
      ...
      estimatedWaitTime = `${
        waitingCount * service.estimated_duration_minutes
      } min`;
    } else {
      waitingCount = queuesForService.filter(
        (q) => q.status === "waiting"
      ...
      if (waitingCount > 0) {
        ...
        callingNumber = queuesForService
          .filter((q) => q.status === "waiting")
          .sort(
            (a, b) => a.queue_number_daily - b.queue_number_daily
          )[0].full_queue_number;
      } else {
  ...
  return publicStatus.sort((a, b) =>
    a.service_prefix.localeCompare(b.service_prefix)
  );
};
...
const emitQueueUpdate = async (serviceId = null) => {
  ...
  console.log(
    "[Socket.IO] Data prepared for emitting:",
    JSON.stringify(dataToEmit, null, 2)
  );
...
const requestOtp = async (req, res) => {
  const { service_id, customer_name, customer_email, customer_phone_number } =
    req.body;
  ...
  try {
    ...
    // Cek apakah email ini sudah punya antrian aktif hari ini.
    const today = new Date();
    const formattedToday = `${today.getFullYear()}-${String(
      today.getMonth() + 1
    ).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
    ...
    const existingActiveQueue = await Queue.findOne({
      where: {
        ...
        queue_date: formattedToday,
        // Status 'aktif' adalah semua status KECUALI yang sudah selesai/gagal.
        status: {
    ...
    // Jika ditemukan antrian yang masih aktif, tolak permintaan.
    if (existingActiveQueue) {
      // Kembalikan error 409 (Conflict), ini lebih spesifik daripada 400.
      return res.status(409).json({
        message:
          "Anda sudah memiliki antrian aktif hari ini. Silakan selesaikan atau batalkan antrian tersebut sebelum mengambil yang baru.",
      });
    }
    // --- [AKHIR LANGKAH VALIDASI BARU] ---

    // Jika tidak ada antrian aktif, proses dilanjutkan...
    const service = await Service.findByPk(service_id);
    if (!service || !service.is_active) {
      return res
        .status(404)
        .json({ message: "Layanan tidak ditemukan atau tidak aktif." });
    }

    // Invalidasi OTP sebelumnya untuk email ini
    await OtpCode.update(
      { is_used: true, is_verified: false, expires_at: new Date() },
      {
        where: {
    ...
    const emailSent = await sendEmailNotification(
      ...
      `Halo ${customer_name}, <br><br>Kode verifikasi Anda adalah: <strong>${otpCode}</strong>. <br><br>Kode ini berlaku selama ${
        process.env.OTP_EXPIRES_MINUTES || 10
      } menit. Jangan bagikan kode ini kepada siapapun.<br><br>Terima kasih.`
    );
    ...
    if (emailSent) {
      ...
      res
        .status(500)
        .json({ message: "Gagal mengirim kode OTP. Silakan coba lagi." });
    }
  } catch (error) {
...
// ... Sisa kode controller Anda sama persis dan tidak perlu diubah ...
// Pastikan semua fungsi lainnya dari file asli Anda tetap ada di sini.
// Saya hanya akan menyalin sisa fungsi Anda agar lengkap.
...
const verifyOtpAndCreateQueue = async (req, res) => {
  ...
  try {
    ...
    if (existingQueue) {
      return res
        .status(409)
        .json({ message: "Anda sudah memiliki antrian aktif hari ini." });
    }
    ...
    if (!otpRecord) {
      return res
        .status(400)
        .json({ message: "Kode OTP tidak valid atau telah kadaluarsa." });
    }
    ...
    if (existingQueuesForServiceToday.length > 0) {
      nextQueueNumberDaily =
        existingQueuesForServiceToday[0].queue_number_daily + 1;
    }
    ...
    const fullQueueNumber =
      service.service_prefix + String(nextQueueNumberDaily).padStart(3, "0");
...
const getMyQueueStatus = async (req, res) => {
  ...
  try {
    ...
    if (!myQueue) {
      return res
        .status(404)
        .json({ message: "No active queue found for this email today." });
    }
    ...
    if (myQueue.status === "pending_otp") {
      return res.status(200).json({
        ...
        current_calling_number: currentCalling
          ? currentCalling.full_queue_number
          : "Belum ada",
        queues_in_front: "N/A",
    ...
    if (myQueue.status === "waiting" || myQueue.status === "on_hold") {
      ...
      const serviceDuration = myQueue.service
        ? myQueue.service.estimated_duration_minutes
        : 10;
...
const requeueMissed = async (req, res) => {
  ...
  if (!customer_email || !service_id) {
    return res
      .status(400)
      .json({ message: "Customer email and service ID are required." });
  }
  ...
  try {
    ...
    if (!missedQueue) {
      return res.status(404).json({
        message: "No missed queue found for this email and service today.",
      });
    }
    ...
    const newQueue = await Queue.create({
      ...
      queue_number_daily:
        (await Queue.count({
          where: { service_id: service_id, queue_date: formattedToday },
        })) + 1,
    });

    const service = await Service.findByPk(service_id);
    if (!service) {
      throw new Error("Service not found for new queue.");
    }
    newQueue.full_queue_number =
      service.service_prefix +
      String(newQueue.queue_number_daily).padStart(3, "0");
    await newQueue.save();
...
const getQueuesForAdmin = async (req, res) => {
  ...
  const formattedToday = `${new Date().getFullYear()}-${String(
    new Date().getMonth() + 1
  ...
  try {
    ...
    const waitingQueues = queuesForService.filter(
      (q) => q.status === "waiting" || q.status === "on_hold"
    );
    ...
    console.error(
      "[Backend QM] Error getting queues for admin (Sequelize):",
      error
    );
    res.status(500).json({ message: "Internal server error." });
  }
...
const callNextQueue = async (req, res) => {
  ...
  try {
    ...
    const notificationThreshold = parseInt(
      setting ? setting.setting_value : "3"
    );
    const currentCallingQueueNumber = nextQueue.queue_number_daily;
    ...
    const upcomingQueues = await Queue.findAll({
      where: {
        ...
        status: "waiting",
        queue_number_daily: {
          [Op.gt]: currentCallingQueueNumber,
        },
      },
      order: [["queue_number_daily", "ASC"]],
    ...
    const calledService = await Service.findByPk(selectedServiceIdInt);
    await sendEmailNotification(
...
const markQueueStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Antrian berhasil ditandai sebagai ${status}.` });
  } catch (error) {
    console.error(
      "[Backend QM] Error marking queue status (Sequelize):",
      error
    );
    res.status(500).json({ message: "Internal server error." });
  }
...
const recallLastCalledQueue = async (req, res) => {
  ...
  try {
    ...
    if (!lastCalled) {
      return res.status(404).json({
        message:
          "Tidak ada antrian yang sedang dipanggil untuk dipanggil ulang.",
      });
    }

    await lastCalled.update({ status: "calling", called_by_user_id: adminId });
    ...
    res.status(200).json({
      ...
      recalled_queue: lastCalled,
    });
  } catch (error) {
    console.error(
      "[Backend QM] Error recalling last called queue (Sequelize):",
      error
    );
    res.status(500).json({ message: "Internal server error." });
  }
...
const getQueueReport = async (req, res) => {
  ...
  if (!startDate || !endDate) {
    return res
      .status(400)
      .json({ message: "Start date and end date are required for reports." });
  }
  ...
  try {
    ...
    const report = await Queue.findAll({
      ...
      attributes: [
        ...
        [
          db.sequelize.fn("COUNT", db.sequelize.col("Queue.id")),
          "total_queues",
        ],
        [
          db.sequelize.fn(
            ...
            db.sequelize.literal(
              "CASE WHEN Queue.status = 'completed' THEN 1 ELSE 0 END"
            )
          ),
          "completed_queues",
        ],
        [
          db.sequelize.fn(
            "SUM",
            db.sequelize.literal(
              "CASE WHEN Queue.status = 'missed' THEN 1 ELSE 0 END"
            )
          ),
          "missed_queues",
        ],
        [
          db.sequelize.fn(
            "SUM",
            db.sequelize.literal(
              "CASE WHEN Queue.status = 'on_hold' THEN 1 ELSE 0 END"
            )
          ),
          "on_hold_queues",
        ],
        [
          db.sequelize.fn(
            "AVG",
            db.sequelize.literal(
              "TIMESTAMPDIFF(MINUTE, Queue.registration_time, Queue.called_time)"
            )
          ),
        ...
        [
          db.sequelize.fn(
            "AVG",
            db.sequelize.literal(
              "TIMESTAMPDIFF(MINUTE, Queue.called_time, Queue.completion_time)"
            )
          ),

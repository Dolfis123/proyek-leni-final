...
const getAllUsers = async (req, res) => {
    try {
        ...
        const users = await User.findAll({
            attributes: ['id', 'username', 'full_name', 'email', 'role', 'is_active', 'created_at'] // Pilih atribut yang ingin ditampilkan
        });
...
const createUser = async (req, res) => {
    ...
    if (!username || !password || !full_name || !email || !role) {
        return res.status(400).json({ message: 'All fields (username, password, full_name, email, role) are required.' });
    }
    if (!['admin'].includes(role)) { // Hanya izinkan membuat role 'admin' dari API ini
...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        if (req.user.role === 'super_admin' && user.role === 'super_admin' && role === 'admin' && user.id !== req.user.id) {
            return res.status(403).json({ message: 'Cannot downgrade another Super Admin role.' });
        }
        if (req.user.role === 'super_admin' && user.id === req.user.id && role && role !== 'super_admin') {

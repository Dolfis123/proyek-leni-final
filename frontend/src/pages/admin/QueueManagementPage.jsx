...
} from "../../api/queue"; // API antrian admin
...
const QueueManagementPage = () => {
    ...
    const fetchServices = async () => {
      ...
      try {
        ...
        const msg =
          ...
          "Gagal memuat layanan untuk manajemen antrian.";
  ...
  const fetchQueues = useCallback(async (serviceId) => {
    ...
    try {
      ...
      console.log(
        `[Admin QM] Fetched queues for service ID ${serviceId}:`,
        data
      ); // DEBUGGING
    } catch (err) {
  ...
  useEffect(() => {
    ...
    console.log(
      "[QueueManagementPage] useEffect (fetchQueues) triggered for selectedServiceId:",
      selectedServiceId
    );
    ...
    if (initialFetchRan.current === false && selectedServiceId) {
      ...
      // Jika sudah bukan initial fetch, panggil fetchQueues saat serviceId berubah
      fetchQueues(selectedServiceId);
    }
  ...
  useEffect(() => {
    ...
    socket.on("queue_update", (data) => {
      ...
      if (selectedServiceId) {
        // Pastikan ada layanan yang dipilih sebelum refresh
        fetchQueues(selectedServiceId);
  ...
  const handleMarkQueueStatus = async (queueId, status) => {
    if (!currentCallingQueue) {
      // Pastikan ada antrian yang sedang dipanggil
      toast.warn("Tidak ada antrian yang sedang dipanggil untuk ditandai.");
      return;
    }
    if (
      !window.confirm(
        `Apakah Anda yakin ingin menandai antrian ini sebagai "${status}"?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success(
        `Antrian berhasil ditandai sebagai ${status.replace("_", " ")}.`
      ); // <<< BARU: Toast sukses
      await fetchQueues(selectedServiceId); // Refresh UI setelah aksi
    } catch (err) {
  ...
  return (
    <DashboardLayout title="Queue Management">
      ...
      {selectedServiceId ? (
        <>
          {loading && (
            <p className="text-blue-500 text-center">Memuat antrian...</p>
          )}
          {/* Pesan error global dari fetchQueues tidak lagi ditampilkan di sini */}
          {/* {error && <p className="text-red-500 text-center">{error}</p>} */}
          {actionLoading && (
            <p className="text-indigo-500 text-center">Memproses aksi...</p>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
              ...
              <div className="text-center">
                <p className="text-6xl font-extrabold text-blue-800 leading-tight">
                  {currentCallingQueue
                    ? currentCallingQueue.full_queue_number
                    : "---"}
                </p>
                {currentCallingQueue && (
                  <p className="text-xl text-gray-600 mt-2">
                    {currentCallingQueue.customer_name}
                  </p>
                )}
              </div>
              <div className="flex justify-center mt-6 space-x-4">
                <button
                  onClick={() =>
                    handleMarkQueueStatus(currentCallingQueue.id, "completed")
                  }
                ...
                <button
                  onClick={() =>
                    handleMarkQueueStatus(currentCallingQueue.id, "missed")
                  }
                  disabled={!currentCallingQueue || actionLoading}
                ...
                <button
                  onClick={() =>
                    handleMarkQueueStatus(currentCallingQueue.id, "on_hold")
                  }
                  disabled={!currentCallingQueue || actionLoading}
            ...
            <div className="bg-white p-6 rounded-lg shadow-md flex flex-col justify-center items-center">
              ...
              {waitingQueues.length === 0 && !loading && (
                    ...
                    {waitingQueues.map((queue) => (
                      <tr key={queue.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() =>
                              handleMarkQueueStatus(queue.id, "on_hold")
                            }
        ...
        !loading && ( // Hapus !error dari kondisi
          <p className="text-gray-600 text-center mt-8">
            Silakan pilih layanan untuk memulai manajemen antrian.
          </p>
        )
      )}
    </DashboardLayout>

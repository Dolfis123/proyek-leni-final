...
// Fungsi ini sudah cukup kuat untuk menangani semua jenis error dari backend,
// termasuk error 409 (Conflict) yang baru kita tambahkan.
const handleError = (error, defaultMessage = "Terjadi kesalahan") => {
  console.error(error); // Log error untuk debugging

  // Mengambil pesan error spesifik dari respon backend jika ada.
  const message =
    error &&
    error.response &&
    error.response.data &&
  ...
  // Melemparkan error baru dengan pesan yang informatif.
...
export const requestOtp = async (data) => {
  try {
    ...
    // handleError akan menangkap pesan "Anda sudah memiliki antrian aktif..."
    // dari backend dan melemparkannya ke komponen frontend.
...
export const getMyQueueStatus = async (email) => {
  try {
    const response = await publicApi.get(
      `/queue/status/my-queue?email=${email}`
    );
    return response.data;
  } catch (error) {
    if (error.response && error.response.status === 404) {
      ...
      return {
        queue: null,
        message:
          error.response.data.message || "Antrian aktif tidak ditemukan.",
      };
    }
...
export const markQueueStatus = async (queueId, status) => {
  try {
    const response = await authApi.put(`/queue/admin/${queueId}/mark-status`, {
      status,
    });
    return response.data;
  } catch (error) {
...
export const recallLastCalledQueue = async (serviceId) => {
  try {
    const response = await authApi.post(
      `/queue/admin/${serviceId}/recall-last`
    );
    return response.data;
  } catch (error) {
